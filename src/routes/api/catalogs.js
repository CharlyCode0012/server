...
router.get("/download", async (req, res) => {
  try {
    ...
    const catalogs = JSON.parse(JSON.stringify(catalogsQuery)).map(
      (catalog) => ({
        id: catalog.id,
        name: catalog.name,
    ...
    // Style each column
    const [idColumn, nameColumn, stateColumn] = [
      worksheet.getColumn("id"),
      worksheet.getColumn("name"),
      worksheet.getColumn("description"),
      worksheet.getColumn("state"),
    ];
    const alignment = { horizontal: "center" };
    [idColumn.alignment, nameColumn.alignment, stateColumn.alignment] = [
      alignment,
      alignment,
      alignment,
    ];
    ...
    for (const catalog of catalogs) worksheet.addRow(catalog);
    ...
    res.setHeader(
      "content-disposition",
      'attachment; filename="Catalogos.xlsx"'
    );
...
router.delete("/", async (req, res) => {
  ...
  try {
    ...
    const Options = await MenuOptions.findAll({
      where: { reference: catalogId },
    });
